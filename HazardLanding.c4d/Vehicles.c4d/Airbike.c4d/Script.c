...
{
	if(GetOwner()==-1) return(0);
	if(!Hostile(GetOwner(pBullet),GetOwner())) return(0);
	if(pRider)
	{
		if(pShooter == pRider) return(0);
		if(LocalN("fSpawnProtected",pRider)) return(0);
	}
	if(pShooter == this()) return(0);
	return(1);
}	
...
func Damage(iDamage){
	if(pRider)
		if(LocalN("fSpawnProtected",pRider)) return(DoEnergy(iDamage));
...
{ 
  ...
  laser->Set(angle,4,700,15,this(),this());
  ...
  // Effekte
  MuzzleFlash(150,this(),x,y,angle,RGBa(96,64,255,0));
