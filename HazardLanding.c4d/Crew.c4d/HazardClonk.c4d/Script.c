...
/*Alternative Schlagaktion und Animation*/
func StartBeatAnim()
{
	//Sound("SwordMiss*");
	return(1);
...
func MA(x,y,r,p)
{
	...
	r += Contents()->~AnimateAngle2();
	if(Contents()->~ NoAnimatedRotate()) r=Contents()->~AnimateAngle2();
	var r2 = Contents()->~AnimateAngle();
	var s = Contents()->~Mirror()*(GetDir()*2-1);//*Contents()->~Mirror();
	if(s==-1) r+=180;
...
func AlternativeBeat()
{
	if(GetAction() eq "Walk" || GetAction() eq "Jump")
		SetAction("AlternativeBeat");
	return(1);
}

protected func WalkPhase()
{
	//if(GetCursor(GetOwner())==this())
	//	InventarAnim();
	if(GetAction()ne"Walk") return(AnimEnd());
	if(Contained()) return(0);
	//Sound("Step*",0,this(),20);
	if(!GetAlive() || fAntiLaag==2 || !Contents())
	{
		SetGraphics (0,this(),BOB1,5,GFXOV_MODE_Base); 
		return(0);
	}
	if(!Contents()->~AnimatedWeapon(this()))
	{
		SetGraphics (0,this(),BOB1,5,GFXOV_MODE_Base); 
		return(0);
	}
	var dir=GetDir(), p=GetPhase();
	if(p>8) p=16-p;
	var dir2=dir*2-1;
	var x=6000, y=1000, r;
	if(p==0){ r=10; x=4; y=2;}// 3 2
	if(p==1){ r=0;  x=3; y=3;}// 3 3
	if(p==2){ r=-10;x=2; y=3;}// 2 2
	if(p==3){ r=-20;x=0; y=3;}// 0 3
	if(p==4){ r=-30;x=-1;y=3;}//-1 3
	if(p==5){ r=-40;x=-2;y=4;}//-2 3
	if(p==6){ r=-50;x=-3;y=4;}//-3 3
	if(p==7){ r=-60;x=-4;y=3;}//-4 3
	if(p==8){ r=-70;x=-4;y=3;}//-4 2
	...
	var r2 = Contents()->~AnimateAngle();
	var s = Contents()->~Mirror()*(GetDir()*2-1);//*Contents()->~Mirror();
	//var s2 = Contents()->~Mirror()*(GetDir()*2-1);
	if(Contents()->~Mirror())
		if(GetDir()==0)
		{
			r-=180;
...
{
	CheckStuck();
	JumpDamage();//Sprungschaden verursachen wenn er auf den Kopf von Gengern springt
	//AnimHelm();
	if(!GetAlive() || fAntiLaag==2 || !Contents())
	{
		SetGraphics (0,this(),BOB1,5,GFXOV_MODE_Base);
	...
	var dir2=dir*2-1;
	var x=-6, y=-3, r=190;
	if(p==0){ r=100; x=-3; y=1;}
	if(p==1){ r=130; x=-4; y=1;}
	if(p==2){ r=150; x=-5; y=0;}
	if(p==3){ r=160; x=-5; y=-1;}
	if(p==4){ r=170; x=-6; y=-1;}
	if(p==5){ r=180; x=-6; y=-2;}
	
	r += Contents()->~AnimateAngle2()-90;
	if(Contents()->~ NoAnimatedRotate()) r=Contents()->~AnimateAngle2();
	var r2 = Contents()->~AnimateAngle();
	var s = Contents()->~Mirror()*(GetDir()*2-1);//*Contents()->~Mirror();
	if(s==-1) r+=180;
...
func AnimEnd()
{
	if(GetAction()eq"Tumble" || GetAction()eq"KneelDown")
		SetGraphics (0,this(),BOB1,6,GFXOV_MODE_Base);
	if(GetAction()ne"Jump" && GetAction()ne"Walk")
	{
		SetGraphics (0,this(),BOB1,5,GFXOV_MODE_Base); 
		return(0);
	}
...
{ 
  var pObj; 
  // NÃ¤chstgelegene Mannschaftsobjekte Ã¼berprÃ¼fen 
  while (pObj = FindObject(0, 0,0,-1,-1, OCF_Alive(), 0, 0, 0, pObj)) 
    // Verfeindet? 
    if (Hostile(GetOwner(),GetOwner(pObj))||pObj->~IsZombie()) 
      // Feind gefunden 
      return(pObj); 
...
/*Mit Schwert oder Axt aufs Fressdeck ballern!*/
func StrikeSound()
{
	Sound("SwordMiss*");
	return(1);
...
{
	//Log("PunchDebug0");
	for(var obj in BashedObjects((GetDir()-1)*25,-10,25,20))
	{
		var pBest = BestWeapon();
		var iS    = pBest->~Strength();
		
		pBest->~Beat(obj);
		if(obj->~HitSound(pBest))
		{
			pBest->~HitSound(obj);
		}
		
		if(!obj->~Hammered(this(),iS))
		{
			//Log("PunchDebug1");
			if(!GetAlive(obj)) obj->~ Hit(this(),iS);
			else Punch(obj,iS);
		}
		//Log("PunchDebug2");
	}
...
func BestWeapon()
{
	var pBest=0;
	var iS=0;
	for(var pObj in FindObjects(Find_Func("Strength"),Find_Container(this())))
	{
		if(pObj->~Strength()>iS)
		{
			pBest = pObj;
...
func BashedObjects(a,b,c,d)
{
	return(FindObjects(Find_Or(Find_InRect(a,b,c,d),Find_Func("InSchlagRange",this(),GetX(),GetY())),Find_Not(Find_Func("NotSchlagable",this())),Find_Or(Find_Func("Schlagable",this()),Find_And(Find_OCF(OCF_Alive),Find_Or(Find_Owner(-1),Find_Hostile(GetOwner()))))));
}
...
protected func ControlSpecial(a,b,c,d)
  {
...
{
	if(pKIObject)
		pKIObject->~OnHit(iDmg,iType,pFrom);
